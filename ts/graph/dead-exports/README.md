...
**Exempt:** package entry points, framework-convention files (`app/`/`pages/` special files, `middleware.ts`, `instrumentation.ts`, `*.config.ts`), files matched by `entryPoints` globs, and test files' own exports (test runners load them directly; they still count as consumers).
