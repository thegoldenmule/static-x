...
The 61 entries collapse to **39 TypeScript tools**; **10 have no TypeScript meaning at all**, and saying precisely why is part of the translation. Eight are shipped, covering ten index entries — [`rename`](ts/refactors/rename/README.md), [the first five](#the-first-five--shipped), [`extract`](ts/refactors/extract/README.md), which retires three entries at once, and [`change-signature`](ts/refactors/change-signature/README.md).
...
Ordered after probing what TypeScript's engine actually does for each candidate, which moved two of them and inverted one. Two are shipped; three remain.
...
| 1 | `ts/refactors/extract-type` | Introduce type alias, Convert Anonymous to Named Type, Introduce typedef | `Extract type` |
| 2 | `ts/refactors/inline-function` | Inline Method — and makes Inline Variable, Inline Field and Inline type alias shippable rather than hazardous | none usable |
| 3 | `ts/refactors/module-form` | Introduce Namespace Alias, partly | `Convert export`, `Convert import` |
