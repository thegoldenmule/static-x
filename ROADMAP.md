...
Ordered after probing what TypeScript's engine actually does for each candidate, which moved two of them and inverted one.
...
| --- | --- | --- | --- |
| 1 | `ts/refactors/extract` | Extract Method, Introduce Variable, Introduce Field | `Extract Symbol` offers all three as *scopes* on one range |
| 2 | `ts/refactors/change-signature` (options-object form) | Transform Parameters, and the beachhead of Change Signature | `Convert parameters to destructured object` |
| 3 | `ts/refactors/extract-type` | Introduce type alias, Convert Anonymous to Named Type, Introduce typedef | `Extract type` |
| 4 | `ts/refactors/inline-function` | Inline Method — and makes Inline Variable, Inline Field and Inline type alias shippable rather than hazardous | none usable |
| 5 | `ts/refactors/module-form` | Introduce Namespace Alias, partly | `Convert export`, `Convert import` |
...
- **`introduce-parameter`** — the composition of 1's selection module and 2's argument mapper. Building it now means building both halves twice.
