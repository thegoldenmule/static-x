...
Ordered after probing what TypeScript's engine actually does for each candidate, which moved two of them and inverted one. Three are shipped; two remain.
...
| 1 | `ts/refactors/inline-function` | Inline Method — and makes Inline Variable, Inline Field and Inline type alias shippable rather than hazardous | none usable |
| 2 | `ts/refactors/module-form` | Introduce Namespace Alias, partly | `Convert export`, `Convert import` |
