...
The 61 entries collapse to **39 TypeScript tools**; **10 have no TypeScript meaning at all**, and saying precisely why is part of the translation. Seven are shipped, covering nine index entries — [`rename`](ts/refactors/rename/README.md), [the first five](#the-first-five--shipped), and [`extract`](ts/refactors/extract/README.md), which retires three entries at once.
...
Ordered after probing what TypeScript's engine actually does for each candidate, which moved two of them and inverted one. The first is shipped; four remain.
...
| 1 | `ts/refactors/change-signature` (options-object form) | Transform Parameters, and the beachhead of Change Signature | `Convert parameters to destructured object` |
| 2 | `ts/refactors/extract-type` | Introduce type alias, Convert Anonymous to Named Type, Introduce typedef | `Extract type` |
| 3 | `ts/refactors/inline-function` | Inline Method — and makes Inline Variable, Inline Field and Inline type alias shippable rather than hazardous | none usable |
| 4 | `ts/refactors/module-form` | Introduce Namespace Alias, partly | `Convert export`, `Convert import` |
...
- **`ts/refactors/signatures.ts`** — the escape/spread-call gate and the `getResolvedSignature` argument mapper, currently private inside `inline-parameter`. Consumed by `change-signature`, then by `introduce-parameter` and `rest-parameter`.
